/*
  ...
  1. New Tables
    - `transactions`
      ...
      - `country_from` (text, default 'BR')
      - `country_to` (text, default 'HT')
      - `receive_value` (numeric, nullable)
      - `receive_currency_iso` (text, nullable)
      - `payment_id` (text, nullable)
      - `payment_method` (text, default 'pix')
      - `refund_id` (text, nullable)
      - `ding_transaction_id` (text, nullable)
      - `distributor_ref` (text, nullable)
  ...
  2. Security
    ...
    - Anon users can view/insert/update transactions
    - Authenticated users can manage own transactions
    - admin_config fully locked down (service_role only)
    - Admin PIN functions use SECURITY DEFINER with restricted grants

  3. Important Notes
    1. Admin PIN seeded as '0330' using bcrypt
    2. Auto-update trigger on transactions.updated_at
    3. Indexes on frequently queried columns (phone, status, created_at, payment_id, ding_transaction_id, user_id, session_id)
*/
